...
This is only needed at the top level of your app, rather than how v5 required wrapping every `<Route/>` you wanted parametrized.
...
Now, Sentry should generate `pageload`/`navigation` transactions with parameterized transaction names (for example, `/teams/:teamid/user/:userid`), where applicable. This is only needed at the top level of your app, rather than how v5 required wrapping every `<Route/>` you wanted parametrized.
